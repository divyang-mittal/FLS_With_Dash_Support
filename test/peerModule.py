...
class PeerModule():
    def foo(PeerService):
        import rpyc
        from rpyc.utils.server import ThreadedServer
        t = ThreadedServer(PeerService, port=self.port, protocol_config={   # for each peer make changes here to set it's port
    ...
    def __init__(self, port, quality, server_port):
        ...
        connection_server = rpyc.connect("localhost", server_port)
        ...
        rc[1].apply(self.foo, PeerService)
        time.sleep(1)
